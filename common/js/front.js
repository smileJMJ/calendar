var calendarMake = (function(){
	...
	fn.initEvent = function(){
		...
		$(document).on("click", $this.ele+" table", function(e){
			...
			if($target.hasClass("badge") || $targetParent.hasClass("badge") || $target.hasClass("no_badge")){		// 일정 클릭
			...
			}else if($target.hasClass("none")){		// 일정 닫기
				$this.hideRegistArea();
			}else{
	...
	fn.calDate = function(year, month, date){
		...
		$this.drawCalendar($this.curYear, $this.curMonth, $this.curAllDate, firstDay);
	};
	...
	fn.drawCalendar = function(year, month, allDate, firstDay){
		...
		for(var i=0; i<rowNum; i++){
			...
			for(var j=0; j<7; j++){				
				if(i==0 && j<firstDay || dCount > allDate) {				// 1일 및 마지막 일 제외한 칸
					...
					if(j==0){
						tbl += "<td class='sunday' data-date='"+$this.curYear+"-"+m+"-"+_numLengthMatch(dCount)+"'><strong>"+dCount+"</strong><div class='no_badge_area'></div></td>";
					}else if(j==6){
						tbl += "<td class='saturday' data-date='"+$this.curYear+"-"+m+"-"+d+"'><strong>"+dCount+"</strong><div class='no_badge_area'></div></td>";
					}else{
						tbl += "<td data-date='"+$this.curYear+"-"+m+"-"+d+"'><strong>"+dCount+"</strong><div class='no_badge_area'></div></td>";
					}
		...
		$calendarTbl.find("td").each(function(i){				// 오늘날짜 today 클래스 추가
			if($(this).data("date") == String($this.toYear+"-"+_numLengthMatch($this.toMonth)+"-"+_numLengthMatch($this.toDate))){
				$(this).addClass("today");
			}
		});
	};
